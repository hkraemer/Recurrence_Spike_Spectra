...
%DIF DEL inter_spike_manuscript_mdpi_old.tex   Wed Aug 31 14:53:00 2022
%DIF ADD inter_spike_manuscript_mdpi.tex       Fri Nov  4 19:49:32 2022
...
%DIF 139-144c108-116
%DIF < \abstract{A novel kind of power spectrum is constructed, the \textit{inter-spike spectrum}, which transforms any signal into its spike-frequency domain. This method 
%DIF < clearly shows the apparent cycles in the data and overcomes the problems for spike-train-like signals when using the obvious idea of Fourier-transforming it. 
%DIF < We invent this instructive approach with the 
%DIF < idea of transforming the $\tau$-recurrence rate of a recurrence plot (RP), which often has a spiky appearance. The $\tau$-recurrence rate is the density of recurrence points 
%DIF < along diagonals of the RP, which are parallel to the main diagonal with a distance of $\tau$. In this context the inter-spike spectrum can be interpreted as a nonlinear power 
%DIF < spectrum of a potentially high dimensional system which constitutes the RP. The proposed measure is robust to noise and is able to detect and analyze bifurcations.}
%DIF -------
 %DIF > 
\abstract{In recurrence analysis, the $\tau$-recurrence rate encodes the periods of the cycles of the underlying high-dimensional time series.  %DIF > 
It, thus, plays a similar role to the autocorrelation for scalar time-series in encoding temporal correlations.  %DIF > 
However, its Fourier decomposition does not have a clean interpretation.  %DIF > 
Thus, there is no satisfactory analogue to the power spectrum in recurrence analysis.  %DIF > 
We introduce a novel way to decompose the $\tau$-recurrence rate using an over-complete basis of Dirac combs together with a sparsity regularization.  %DIF > 
We show that this decomposition, the \textit{inter-spike spectrum}, naturally provides an analogue to the power spectrum for recurrence analysis in the sense that it reveals the dominant periodicities of the underlying time series.  %DIF > 
We show that the inter-spike spectrum correctly identifies patterns and transitions in the underlying system in a wide variety of examples and is robust to measurement noise.} %DIF > 
 %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{Recurrence Plots }\DIFdelend \DIFaddbegin \DIFadd{The dynamics of complex systems as provided by measured time series usually show complicated and chaotic patterns.
Quantifying their recurrence features is a powerful way to describe them and to infer information about 
the type of dynamics, stability, regime changes, or couplings and synchronisation \mbox{%DIFAUXCMD
\cite{marwan2007,marwan2008epjst,webber2015}}\hskip0pt%DIFAUXCMD
.
Even more challenging are signals which do have a heavy tailed-distribution or appear as a spike-train,
e.g., neuron firings \mbox{%DIFAUXCMD
\cite{Dummer2014,Orcioni2020}}\hskip0pt%DIFAUXCMD
, heart beat variability \mbox{%DIFAUXCMD
\cite{marwan2002herz}}\hskip0pt%DIFAUXCMD
, 
or extreme flood events \mbox{%DIFAUXCMD
\cite{banerjee2021}}\hskip0pt%DIFAUXCMD
.
Deriving useful information from spike-train signals or inter-spike time series is an important
topic in data analysis in many scientific fields \mbox{%DIFAUXCMD
\cite{Kajikawa2005,Dummer2014,Orcioni2020,Canale2021}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{Recurrence plots }\DIFaddend (RPs) provide a vivid representation of complex dynamics \DIFaddbegin \DIFadd{$\vec{x}_i$ }\DIFaddend stemming from potentially high dimensional systems \cite{marwan2007}
\begin{equation}\label{eq_rp_definition}
R_{i,j}(\varepsilon) = \Theta\left(\varepsilon - D_{i,j}\right) 
= \Theta\left(\varepsilon - \| \vec{x}_i - \vec{x}_j\|\right), \qquad \vec{x} \in \mathbb{R}^d, \quad i,j \in [1,\ldots, N]\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
\DIFaddbegin \DIFadd{with $\mathbf{R}$ the recurrence matrix, $\vec{x}_i$ the state vector at time 
...
}\DIFaddend The crucial free parameter $\varepsilon$ \DIFdelbegin \DIFdel{(recurrence threshold) determines }\DIFdelend \DIFaddbegin \DIFadd{is the recurrence threshold, determining what is a recurrences
and, thus, }\DIFaddend the visible structures in the RP. It can be chosen such that the recurrence rate 
...
\DIFaddend A rather simple idea is to look at the $\tau$-recurrence rate of the RP ($\tau$-RR, Eq.~\ref{eq_tau_rr}) \cite{marwan2002pla,Zbilut2008}.
...
\DIFaddbegin \DIFadd{It represents the period length of cycles in the data (Fig.~\ref{fig_tau_rr_spectrum_example}D).
}

\DIFaddend \citet{Zbilut2008} pointed out that $\tau$-RR could be \DIFdelbegin \DIFdel{used as a plugin value for }\DIFdelend \DIFaddbegin \DIFadd{interpreted as analogous to }\DIFaddend the auto-correlation function $C(\tau)$ and, hence, via the \DIFdelbegin \DIFdel{Wiener-Khinchim theorema 
}\DIFdelend \DIFaddbegin \DIFadd{Wiener-Khinchin theorem, provide an analogue,
}\DIFaddend ``generalized''\DIFdelbegin \DIFdel{powerspectrum can be obtained}\DIFdelend \DIFaddbegin \DIFadd{, spectral density}\DIFaddend . This is reasonable, since the average distances for a given lag $\tau$ 
\begin{equation}
\overline{D}(\tau) = \frac{1}{N-\tau}\sum_{i=1}^{N-\tau} D_{i, i+\tau}
\end{equation}
can be directly read from the distance matrix $\mathbf{D}$ and is also preserved in its thresholded version $\tau$-RR. There are clear advantages for a recurrence-derived \DIFdelbegin \DIFdel{powerspectrum}\DIFdelend \DIFaddbegin \DIFadd{spectral density}\DIFaddend , 
...
Furthermore, \DIFaddbegin \DIFadd{since an RP can represent high-dimensional dynamics and its $\tau$-RR serves as a plug-in for $C(\tau)$, }\DIFaddend the correlation structures of higher dimensional spaces can be \DIFdelbegin \DIFdel{resolved in }\DIFdelend \DIFaddbegin \DIFadd{read from 
}\DIFaddend the recurrence-derived Fourier-spectrum.\\
...
However, \DIFdelbegin \DIFdel{there is a drawback to this approach. }\DIFdelend \DIFaddbegin \DIFadd{the interpretation of this generalized power spectral density is unclear, and it is typically hard to interpret. $\tau$-RR directly encodes the periodicity of the underlying signal, in contrast it is unclear what, if any, interpretation the "power" contained in a particular frequency mode of the $\tau$-RR should have. }\DIFaddend Whenever $\tau$-RR is a spike-train-like signal, which it is in most cases (see Fig.~\ref{fig_tau_rr_spectrum_example}) especially for 
...
Fig.~\ref{fig_tau_rr_spectrum_example}E). Thus, it is not intuitive how to extract meaningful information about \DIFdelbegin \DIFdel{dominant frequencies of }\DIFdelend the systems' state space trajectory. 

For \DIFdelbegin \DIFdel{clarification}\DIFdelend \DIFaddbegin \DIFadd{example}\DIFaddend , consider the signal we would like to analyze 
...
in principle we would strive for a single peak in the frequency domain of this signal at a frequency $f=1/T_\text{is}$. \DIFdelbegin \DIFdel{Surprisingly the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend Fourier spectrum does not meet this expectation 
and instead of a single frequency, there are \DIFdelbegin \DIFdel{exceptionally }\DIFdelend many frequencies excited (Fig.~\ref{fig_tau_rr_dirac_comb}B, E). This is because the Fourier components add constructively 
for every frequency $1/T_\text{is}$ and therefore $\text{DC}_{\text{is}}(t)$ coincides with its own Fourier transform up to a factor $1/T_\text{is}$ \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Norden1998}}\hskip0pt%DIFAUXCMD
.}\footnote{\DIFadd{This can also 
be observed for neuron spike trains, e.g. \mbox{%DIFAUXCMD
\cite{Orcioni2020,Biagetti2017}}\hskip0pt%DIFAUXCMD
.}}
\DIFaddend 
...
\begin{figure}[h]
 ...
 \textbf{B} its FFT-based \DIFdelbeginFL \DIFdelFL{powerspectrum}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{power spectral density}\DIFaddendFL .
 ...
 \textbf{E} its FFT-based \DIFdelbeginFL \DIFdelFL{powerspectrum}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{power spectral density}\DIFaddendFL . 
...
In this article we propose a new way of \DIFdelbegin \DIFdel{transforming }\DIFdelend \DIFaddbegin \DIFadd{decomposing }\DIFaddend a spike-train-like signal into \DIFdelbegin \DIFdel{its frequency domain}\DIFdelend \DIFaddbegin \DIFadd{periodic components as 
an alternative to the RP-based method suggested in \mbox{%DIFAUXCMD
\cite{Zbilut2008} }\hskip0pt%DIFAUXCMD
or Fourier-based spike-train power 
spectra \mbox{%DIFAUXCMD
\cite{Dummer2014}}\hskip0pt%DIFAUXCMD
}\DIFaddend . This novel \textit{inter-spike spectrum} does not show resonance 
behavior of the signal's inherent inter-spike frequencies \DIFaddbegin \DIFadd{in such a way that harmonics of these frequencies are also excited }\DIFaddend (Fig.~\ref{fig_tau_rr_dirac_comb}C, F). Section \ref{sec_tau_rr_method} explains the idea\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{. Note that this approach }\DIFaddend can be used to decompose \DIFdelbegin \DIFdel{any arbitrary signal }\DIFdelend \DIFaddbegin \DIFadd{arbitrary signals, }\DIFaddend and is not \DIFdelbegin \DIFdel{restricted to the }\DIFdelend \DIFaddbegin \DIFadd{specific to }\DIFaddend $\tau$-RR\DIFdelbegin \DIFdel{, which we exemplify here}\DIFdelend . However, the more spiky the signal is, the more \DIFdelbegin \DIFdel{outstanding }\DIFdelend \DIFaddbegin \DIFadd{useful }\DIFaddend our new approach is compared to a 
FT. In Section \ref{sec_tau_rr_application} we \DIFdelbegin \DIFdel{exemplify }\DIFdelend \DIFaddbegin \DIFadd{demonstrate }\DIFaddend the usage of inter-spike spectrum when transforming the $\tau$-RR of a system under study. In this case, the inter-spike spectrum 
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{To obtain the inter-spike spectrum, the }\DIFaddend signal, which we would like to \DIFdelbegin \DIFdel{transform }\DIFdelend \DIFaddbegin \DIFadd{analyse, in our case the $\tau$-RR, }\DIFaddend is decomposed into a set of appropriate basis functions. 
\DIFdelbegin \footnote{\DIFdel{We focus on $\tau$-RR in this article, Eq.~\eqref{eq_tau_rr}, since 
such time series are often spiky, which favors the proposed decomposition technique. But the proposed method can be applied to any sort of signal.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{Instead of using trigonometric functions , as it is the idea in the Fourier decomposition , we }\DIFdelend \DIFaddbegin \DIFadd{The general idea common to many methods is that the sum of these weighted functions can approximate a finite signal to a sufficient degree. The weights (in some contexts also called modes or loadings) corresponding to the individual basis functions must be determined.
A number of decomposition techniques based on different sets of basis functions exist, e.g., 
trigonometric functions (Fourier- and wavelet analysis \mbox{%DIFAUXCMD
\cite{Bracewell1986}}\hskip0pt%DIFAUXCMD
), 
eigenvectors of the corresponding covariance matrix (principal component analysis \mbox{%DIFAUXCMD
\cite{Hotelling1933} }\hskip0pt%DIFAUXCMD
and related techniques) 
or intrinsic mode functions (empirical mode decomposition and Hilbert spectrum \mbox{%DIFAUXCMD
\cite{Norden1998}}\hskip0pt%DIFAUXCMD
).
...
\DIFdelbegin %DIFDELCMD < \noindent %%%
\DIFdelend Eventually, an under-determined linear system
\begin{equation}
...
(LASSO) \cite{Tibshirani1996} or a \textit{sequentially thresholded least squares} (STLS) regression \cite{Brunton2016,Brunton2019} to obtain a solution $\hat{\boldsymbol\beta}$. \DIFaddbegin \DIFadd{Any other 
sparse regression technique could be used. }\DIFaddend Finally, we group loadings 
...
\DIFdelbegin \DIFdel{In addition to the computational complexity of solving Eq.~\eqref{eq_linear_system}, the solving algorithm must also deal with the free choice of the }\DIFdelend \DIFaddbegin \DIFadd{When introducing a sparsity regularization we give up the ability to fully reconstruct the signal from the decomposition in favor of identifying the most prominent spikes, and, thus, the most important periodicities more clearly. This choice is parametrized by a }\DIFaddend regression regularization parameter $\alpha$\DIFdelbegin \DIFdel{, 
...
since $\alpha=0$ corresponds to the least squares solution and the corresponding $\hat{\boldsymbol\beta}$ is an exact solution of }\DIFdelend \DIFaddbegin \DIFadd{. Only for $\alpha = 0$ do we obtain a solution to }\DIFaddend Eq.~(\ref{eq_linear_system}) and\DIFdelbegin \DIFdel{not sparse. On the other hand, only a sparse number of non-zero loadings in $\hat{\boldsymbol\beta}$, i.e., peaks in the inter-spike spectrum, are obtained when }\DIFdelend \DIFaddbegin \DIFadd{, thus, a full reconstruction of the signal. Our general approach to fixing }\DIFaddend $\alpha$ is \DIFdelbegin \DIFdel{sufficiently large. 
The actual implementation of the regularization depends on the regression method chosen }\DIFdelend \DIFaddbegin \DIFadd{to require that we preserve most of the original signal in the sense of matching a given (Pearson) correlation coefficient $\rho$ between the original signal $\vec{s}$ and the reconstruction. The chosen regression method }\DIFaddend (in our case, LASSO or STLS) \DIFdelbegin \DIFdel{, and consequently a given value of $\alpha$ will generally result in different sparse loading vectors $\hat{\boldsymbol\beta}$.
Therefore, in this paper we select }\DIFdelend \DIFaddbegin \DIFadd{will determine how sensitive the obtained inter-spike spectrum is with respect to the regularization given by a 
...
The proposed decomposition does not show a leakage effect by construction. However, when introducing a sparsity regularization we give up the ability to fully reconstruct the signal from the decomposition in favor of identifying the most important periodicities more clearly. This choice is mediated by a regression regularization parameter }\DIFaddend $\alpha$\DIFdelbegin \DIFdel{such that the re-composed signal $\tilde{\vec{s}}=\bfX^T\hat{\boldsymbol\beta}$ matches }\DIFdelend \DIFaddbegin \DIFadd{. Only for $\alpha = 0$ do we obtain a solution to Eq.~(\ref{eq_linear_system}) and thus a full reconstruction of the signal. Our general approach to fixing $\alpha$ is to require that we preserve most of the original signal in the sense of matching }\DIFaddend a given (Pearson) correlation coefficient $\rho$ between the original signal $\vec{s}$ and \DIFdelbegin \DIFdel{itself. }\DIFdelend \DIFaddbegin \DIFadd{the reconstruction. The chosen regression method (in our case, LASSO or STLS) will determine how sensitive the obtained inter-spike spectrum is with respect to the regularization given by a 
selected $\rho$ (Fig.~\ref{fig_regularization}). Regarding this, in most of our applications LASSO yields more robust results, while STLS is usually faster to compute. 
We will discuss this in more detail in Section \ref{sec_tau_rr_discussion}.
}\DIFaddend 

%DIF > 
%DIF > In addition to the computational complexity of solving Eq.~\eqref{eq_linear_system}, the solving algorithm must also deal with the free choice of the regression regularization parameter $\alpha$, 
%DIF > which determines the sparsity of the regression coefficient vector $\hat{\boldsymbol\beta}$. Obviously, there will be numerous peaks in the inter-spike spectrum of a given signal, when $\alpha$ is small, 
%DIF > since $\alpha=0$ corresponds to the least squares solution and the corresponding $\hat{\boldsymbol\beta}$ is an exact solution of Eq.~(\ref{eq_linear_system}) and not sparse. On the other hand, 
%DIF > only a sparse number of non-zero loadings in $\hat{\boldsymbol\beta}$, i.e., peaks in the inter-spike spectrum, are obtained when $\alpha$ is sufficiently large. 
%DIF > The actual implementation of the regularization depends on the regression method chosen (in our case, LASSO or STLS), and consequently a given value of $\alpha$ will generally result in 
%DIF > different sparse loading vectors $\hat{\boldsymbol\beta}$. Therefore, in this paper we select $\alpha$ such that the re-composed signal $\tilde{\vec{s}}=\bfX^T\hat{\boldsymbol\beta}$ matches a 
%DIF > given (Pearson) correlation coefficient $\rho$ between the original signal $\vec{s}$ and itself.
\DIFaddbegin 
...
\DIFdelbegin \DIFdel{While }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig_roessler_peaks} illustrates that 
while }\DIFaddend the peak shape does change in the presence of noise, its position does not, and this is what the inter-spike spectrum encrypts after all 
\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(see Fig.~\ref{fig_roessler_spectra_distances} in the Appendix for further analysis).
}\DIFaddend 

\DIFaddbegin \begin{figure}
 \centering
 \includegraphics[width=0.9\textwidth]{./figures/fig_roessler_different_noise_levels_peaks}
 \caption{\DIFaddFL{Peak positions of the obtained inter-spike spectra of the $\tau$-RRs for additive noise levels up to 50\% for the discussed R\"ossler dynamics, 
 }\textbf{\DIFaddFL{A}} \DIFaddFL{period-2 limit-cycle, }\textbf{\DIFaddFL{B}} \DIFaddFL{period-3 limit-cycle and }\textbf{\DIFaddFL{C}} \DIFaddFL{chaos. The size of the plotted markers scale with the detected peak height. The noise-free spectra are shown in Fig.~\ref{fig_tau_rr_example_roessler}J, K, L and 
...
\begin{figure}
 ...
 \caption{Averaged \DIFaddbeginFL \DIFaddFL{evolutionary }\DIFaddendFL Fourier power spectra of recorded power grid frequency time series of \textbf{A} Central Europe (CE) and \textbf{B} Great Britain (GB) (see 
 \DIFaddbeginFL \DIFaddFL{Fig.~\ref{fig_power_grid_time_series} in }\DIFaddendFL Appendix~\ref{sec_power_grid_appendix}). 
 ...
 $15$, $30$ and $60$ \si{min}. For technical details on the calculation of the spectra shown, \DIFaddbeginFL \DIFaddFL{i.e preprocessing and window size being used, }\DIFaddendFL the reader is referred to the Appendix~\ref{sec_power_grid_appendix}.}  
\label{fig_power_grid_spectra}
\end{figure}

The \DIFdelbegin \DIFdel{Fourier frequency spectra }\DIFdelend \DIFaddbegin \DIFadd{evolutionary Fourier frequency spectrum }\DIFaddend for the Central Europe data in Figure~\ref{fig_power_grid_spectra}A, however, does not display sharp peaks exactly at $15$, $30$ and $60$ \si{min}\DIFaddbegin \DIFadd{, which may partly 
be explained by the leakage effect }\DIFaddend (for technical details on the calculation of 
...
\DIFaddbegin \DIFadd{The fact that sharp peaks are seen here using the same window size and sampling frequency as in the case of the Central European data shows that the leakage effect alone is not the cause of 
the peak splitting shown in Fig.~\ref{fig_power_grid_spectra}A.
}

\DIFaddend In contrast, the $15$ \si{min} peak is completely absent from the inter-spike spectra of the $\tau$-RRs of the frequency data (Fig.~\ref{fig_power_grid_spectra}C,D), but these show sharp peaks 
at $30$ and $60$ \si{min} \DIFaddbegin \DIFadd{(there is no leakage effect due to the proposed decomposition technique)}\DIFaddend . 
...
\begin{figure}
 \centering
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{./figures/fig_laskar_spectra}
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/fig_laskar_spectra}
 ...
 series and \DIFdelbeginFL \textit{\DIFdelFL{C}} %DIFAUXCMD
...
This is not a drawback of the proposed \DIFaddbegin \DIFadd{decomposition }\DIFaddend method, but rather a drawback of \DIFdelbegin \DIFdel{the particular application method that we have heavily used in this article and }\DIFdelend \DIFaddbegin \DIFadd{applying this technique to the $\tau$-recurrence rate of the system, }\DIFaddend which was the main motivation for 
...
A novel \DIFdelbegin \DIFdel{type of powerspectrum, }\DIFdelend \DIFaddbegin \DIFadd{decomposition technique is proposed that yields }\DIFaddend the \textit{inter-spike spectrum}\DIFdelbegin \DIFdel{, has been proposed}\DIFdelend . The method decomposes any arbitrary signal into basis functions which consist of (lagged) Dirac 
combs (DC) of different inter-spike period. The loading for each period is obtained by a regularized regression, which promotes sparsity in its solution. \DIFdelbegin \DIFdel{Wee }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend chose LASSO or a sequentially 
...
of the period (or frequency) the inter-spike spectrum is obtained. \DIFaddbegin \DIFadd{A disadvantage is that the transformation is not invertible. An advantage is that there is no leakage effect.
}\DIFaddend Although this novel \DIFdelbegin \DIFdel{power }\DIFdelend spectrum is superior to an ordinary FFT-based power spectrum when the signal has a spike-train-like appearance, the authors suggest that this method should 
be considered 
...
%DIFDELCMD < All computations have been carried out in MATLAB\textsuperscript{\textregistered} and the Julia language 
%DIFDELCMD < and made use of the packages \textit{DynamicalSystems.jl} \cite{datseris2018} and \textit{DifferentialEquations.jl} \cite{rackauckas2017}.
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFaddbegin \begin{figure}
 ...
 \caption{\DIFaddFL{Fourier spectra of the frequency entire time series from }\textbf{\DIFaddFL{A}} \DIFaddFL{Central Europe (CE), $N_{\text{CE}}=587,520$ and }\textbf{\DIFaddFL{B}} \DIFaddFL{Great Britain (GB), $N_{\text{GB}}=1,581,120$. A subset of the underlying time series is shown in Fig.~\ref{fig_power_grid_time_series}.}}
...
\begin{figure}[h!]
 ...
 \caption{\DIFaddFL{Wasserstein distances for the obtained inter-spike spectra of the $\tau$-RRs for additive noise levels up to 50\% for the discussed R\"ossler dynamics, 
 }\textbf{\DIFaddFL{A}} \DIFaddFL{period-2 limit-cycle, }\textbf{\DIFaddFL{B}} \DIFaddFL{period-3 limit-cycle and }\textbf{\DIFaddFL{C}} \DIFaddFL{chaos. For each noise level the obtained spectrum is compared to the noise-free spectrum (these are shown 
