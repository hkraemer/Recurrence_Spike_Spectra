...
\abstract{In reccurance analysis, the $\tau$-recurrence rate encodes crucial information on the cycles of the underlying high-dimensional timeseries. It plays a similar role to the autocorrelation for scalar time-series in encoding temporal correlations, however, it is not amenable to analysis by Fourier decomposition. Thus there is no natural analogue to the power spectrum in recurrence analysis. In this paper we introduce a novel way to decompose the $\tau$-recurrence rate using an overcomplete basis of Dirac combs together with a sparsity regularization. We show that this decomposition, the \textit{inter-spike spectrum}, naturally provides an analogue to the power spectrum for recurrence analysis. We show that the inter-spike spectrum correctly identifies patterns and transitions in the underlying timeseries in a wide variety of examples and is robust to measurement noise.}
%
