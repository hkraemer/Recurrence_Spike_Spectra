...
\begin{figure}
 ...
 which is highlighted in pink in \textbf{A}.\todo[inline]{MA: I think Fig.~\ref{fig_tau_rr_spectrum_example} should be restructured. We could talk about it.}
...
($\rho_{\text{Pearson}}=0.72$). Moreover, this analysis can tackle period-doubling, since it ``measures'' the dominant cycles via the inter spike spectrum. \todo{MA: This part is not understandable}
...
\begin{figure}
 ...
 \caption{\textbf{A} and \textbf{B} demonstrate the power spectrum of recorded power grid frequency data from ... and ..., respectively. The data has been shown in Fig.~\ref{} in Appendix ...\textbf{C} and \textbf{D} show the corresponding inter-spike- spectrum.}
...
\item STLS and LASSO yield different results.
\item incorporates FFT-peaks (see Vostok data).
\item bifurcation detection only possible when additional period(s) is not an integer multiple of the former period(s).
\end{itemize}
