...
\Author{K. Hauke Kraemer $^{1,}$*\orcidA{}, Frank Hellmann $^{1,}$, Mehrnaz Anvari $^{1}$, J\"urgen Kurths $^{1,2,4}$ and Norbert Marwan $^{1,3,}$*}
...
\end{align}
\todo[inline]{HK: Eq.\eqref{eq_basis_matrix} is somehow ugly. Maybe someone can get rid of that sum and make it nicer? I tried with 
modulo, but just could not figure it out exactly. There must be an easy way I do not see.}
...
eccentricity cycles of 405 kyr period, 95 kyr period and 124 kyr period are well known in paleoclimate studies \cite{Laskar2004}. Our aim in this section is to show that the inter spike spectrum 
...
the corresponding Fourier spectrum (cf. Section \ref{sec_tau_rr_intro}). We use a time series which covers the past \mytilde $67$ Mio. years, which a total length of $N=13,421$ samples and a 
sampling period of $\Delta t= 5,000$.\\
First we compute an evolutionary short time Fourier transform (FT) using a windowsize of $ws=1,000$ samplepoints ($\equiv 5 Mys$) shifted by unity and a 
Hamming window. The spectrogram is shown in Figure \ref{fig_laskar_spectra}A, where the expected periods mentioned and highlighted are clearly visible. 
The spectrogram is shown in Figure A, where the expected periods mentioned and highlighted are clearly visible.  
...
and the inter spike spectra of the corresponding $\tau$-RR's is obtained by using STLS regression (see Section \ref{sec_tau_rr_method}) and a regularization threshold corresponding to 
$\rho=0.9$ accordance of $\tau$-RR's and re-composed signals. We only use the first $200$ data points of the $\tau$-RR (covering a time span of $1$ Myrs). 
The spectrogram shown in Figure \ref{fig_laskar_spectra}B also highlights the $95$, $124$ and $405$ kyrs periods as expected. 
Additional power is distributed in its harmonics at $190$ and $248$ kyrs periods. Finally, the standard FT with a Hamming window of the same $\tau$-RR's used to obtain the inter spike 
...
As described in Section \ref{sec_tau_rr_method} our idea in this paper is to select the $\alpha$ such that the re-composed signal $\tilde{\bf{s}}=\bf{X}^T\hat{\boldsymbol\beta}$ matches a 
given (Pearson) correlation coefficient $\rho_{\bf{s},\tilde{\bf{s}}}$ between the original signal $\bf{s}$ and itself. This ensures that $\alpha$ adjusts itself to the data as well as to the 
...
However, the two different sparse regression algorithms we encountered in this article (LASSO and STLS) yield different results for the same desired $\rho_{\bf{s},\tilde{\bf{s}}}$. 
...
is that $\rho_{\bf{s},\tilde{\bf{s}}}$ is not a smooth function of $\alpha$ in case of STLS, due to the hard-thresholding involved \cite{Brunton2016}, which is shown in Figure~\ref{fig_regularization}.
...
Although this novel power spectrum is superior to an ordinary FFT-based power spectrum when the signal has a spike-train-like appearance, the authors suggest that this method be considered 
as an additional source of information and not as a substitute for ordinary Fourier analysis. 
...
\begin{figure}[h!]
 ...
 \caption{Pearson correlation coefficient $\rho_{\bf{s},\tilde{\bf{s}}}$ between the input signal signal $\bf{s}$ and the re-composed signal $\tilde{\bf{s}}=\bf{X}^T\hat{\boldsymbol\beta}$ as a 
