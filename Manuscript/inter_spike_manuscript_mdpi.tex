...
Unlike the autocorrelation however, its Fourier decomposition does not have a natural interpretation.
Thus there is no direct analogue to the power spectrum in recurrence analysis.
In this paper we introduce a novel way to decompose the $\tau$-recurrence rate using an over-complete basis of Dirac combs together with a sparsity regularization.
...
and since an RP can represent high dimensional dynamics the approch directly applies to higher dimensional systems for which the auto-correlation is a \todo{hm, I'm not so happy with the "matrix" here} matrix and no single power spectrum captures the recurrences.

\todo[inline]{I agree that the following is not necessary and could be removed}
...
sparse regression technique could be used {\color{red} (I think this sentence can be the source of confusion. One could get from this sentence that weight values are independent of regression technique. Is it correct?)}. Finally, we group weights 
...
%only a sparse number of non-zero weights in $\hat{\boldsymbol\beta}$, i.e., peaks in the inter-spike spectrum, are obtained when $\alpha$ is sufficiently large. 
