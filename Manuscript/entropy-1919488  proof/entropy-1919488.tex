...
\Author{\hl{K. Hauke Kraemer} %MDPI: Please carefully check the accuracy of names and affiliations. %Authors: we agree
 $^{1,}$*\orcidA{}, Frank Hellmann $^{1}$, Mehrnaz Anvari $^{1}$\orcidB{}, \hl{J\"urgen Kurths} %MDPI: The name of this author is different from the submitting system. Please confirm which one is correct. %Authors: we agree
...
\address{%
$^{1}$ \quad Potsdam Institute for Climate Impact Research (PIK), Member of the Leibniz Association, 14473 Potsdam, Germany; \hl{hellmann@pik-potsdam.de (F.H.); anvari@pik-potsdam.de (M.A.); kurths@pik-potsdam.de (J.K.)} %MDPI: We added the email addresses here according to the submitting system. Please confirm.%Authors: we agree
\\
$^{2}$ \quad Institute of Physics and Astronomy, University of Potsdam, 14476 Potsdam, Germany\\
\hl{$^{3}$} %MDPI: The affiliation numbers should appear in numerical order. Please check the suggested changes. %Authors: we agree
...
\end{equation}
where $\mathbf{R}$ \hl{is} %MDPI: There is no bold in other places in the text, please confirm whether the bold here needs to be retained %Authors: we confirm that this is boldface, because it this variable represents a matrix
...
\begin{figure}[H]
 ...
 \caption{\hl{Schematic} %MDPI: Please change the hyphen (-) into minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1". %Authors: we changed to minus sign
...
to every frequency $1/T_\text{is}$; therefore, $\text{DC}_{\text{is}}(t)$ coincides with its own Fourier transform up to a factor $1/T_\text{is}$ \cite{Norden1998}. ({\hl{This can also } %MDPI: footnote is not allowed in this journal, we moved here, please confirm  %Authors: we agree/ confirm
...
% \centering
 ...
 \caption{\hl{Inter-spike spectra} %MDPI: Please use a consistent numbering style for the subfigures throughout the paper. P is after N;  Please change the hyphen (-) into minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1". %Authors: We changed the hyphen to minus sign. We have modified the panel numbering to make it consistent with the other figures.
...
\hl{In this example} %MDPI: we add indent here, please confirm %Authors: we agree/ confirm
...
\newblock {JuliaOptimalTransport/OptimalTransport.jl: v0.3.19}%MDPI: Please provide more information about the type of the article, such as book (please provide the name of the publisher and location of it), online resource (please provide the URL of the website and the date it was accessed (day, month, year)), or journal article (please provide the name of the journal, the year and volume it published, and the page number). %Authors: we confirm that we hold the copyright of this figure
. {2022}.
\newblock
   \hl{Available online:} %MDPI: we replace the correct web link, please confirm
 \url{https://zenodo.org/record/5920148\#.Y3SbbORByUk} \hl{(accessed on)} %MDPI: Please add accessed date.
...
\newblock  \hl{Available online}%MDPI: please add the website link.
