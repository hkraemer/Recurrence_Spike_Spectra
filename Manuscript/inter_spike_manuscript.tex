...
\institute{K. Hauke Kraemer \and J\"urgen Kurths \and Norbert Marwan \and Frank Hellmann \and Mehrnaz Anvari \at Potsdam Institute for Climate Impact Research, Member of the Leibniz Association, 14473 Potsdam, 
...
\cite{Zbilut2008} pointed out that $\tau$-RR could be used as a plugin value for the auto-correlation function $C(\tau)$ and, hence, via the Wiener-Khinchim theorem a 
...
FT. In Section \ref{sec_tau_rr_application} we exemplify \sout{its use} \todo{the usage of inter spike spectrum} when transforming the $\tau$-RR of a system under study. In this case, the inter spike spectrum can unravel 
characteristic time scales of high dimensional systems, which is not possible when using a FT. \todo{Finally, in Section \ref{sec_tau_rr_conclusion} our results are summarized} 

\section{Method}\label{sec_tau_rr_method}
    
The signal, which we would like to transform -- we focus on $\tau$-RR in this article, but this can be applied to any sort of signal \todo{what do you mean with any signal? do you mean to look at data set instead of $\tau$-RP or RP?}-- 
...
different inter-spike periods as basis functions, Eq.~\eqref{eq_dirac_comb}. Let $s(t_i)$ be the normalized signal \todo{which} we want to transform. \sout{with}\todo{The signal has} length $N$ and 
...
(Fig.~\ref{fig_tau_rr_basis_functions}A illustrates the described procedure) \todo{$T$ in the following eqs. should not be $T_{is}$ to be consistent with the text?}\todo{$n$ in eq.6 is not clear for me}
\begin{align}
...
In addition to the computational complexity of solving Eq.~\eqref{eq_linear_system}, the proposed method \todo{which method? STLS?}must also deal with the free choice of the regression regularization parameter $\alpha$, 
...
of non-zero loadings in $\hat{\bf{\beta}}$ -- \textit{aka} \todo{what is aka?}peaks in the inter spike spectrum -- are obtained when $\alpha$ is sufficiently large. In this paper we select $\alpha$ such that the 
...
\todo[inline]{why you have not used STLS method here?} }
...
\subsection{Power grid frequency time series}\label{sec_power_grid_appendix}
